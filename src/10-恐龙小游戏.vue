...
const runTextures = [];
for (let i = 0; i < 2; i++) {
	runTextures.push(new PIXI.Texture(baseTexture, new PIXI.Rectangle(1680 + (2 + i) * 88, 0, 82, 100)));
}
...
app.ticker.add(() => {
	...
	// 检测碰撞
	if (jumpSprite.y > cactusSprite.y - 60 && cactusSprite.x < jumpSprite.x + 60 && cactusSprite.x > jumpSprite.x - 60) {
		// 游戏结束
		gameOver();
		startText.visible = false;
